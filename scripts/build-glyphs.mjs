...
/**
 * Traced handwriting fonts come from the fonts repo (github.com/ghostmind-labo/fonts), which stores
 * each character as pen strokes. They are baked in here at build time — a board renders in a
 * sandbox with no network, so nothing can be fetched later.
 *
 * Sources are tried in order, and the first one present wins. Both are explicit on purpose: a build
 * never picks up fonts nobody asked for, so what ships is whatever the checkout declares.
 *   1. SKETCH_FONTS=<dir>  — a path to the repo's fonts/ directory (a local clone while working on a font)
 *   2. node_modules/@ghostmind-dev/fonts/fonts — the published package, when it is a dependency
 */
const FONT_SOURCES = [process.env.SKETCH_FONTS, 'node_modules/@ghostmind-dev/fonts/fonts'].filter(Boolean);
const fontDir = FONT_SOURCES.find((dir) => existsSync(dir));
for (const dir of fontDir ? readdirSync(fontDir, { withFileTypes: true }).filter((d) => d.isDirectory()) : []) {
  const file = join(fontDir, dir.name, 'glyphs.json');
  if (!existsSync(file)) continue;
  const glyphs = JSON.parse(readFileSync(file, 'utf8'));
  if (!glyphs[' ']) glyphs[' '] = [300, ''];
  if (glyphs['-'] && !glyphs['−']) glyphs['−'] = glyphs['-'];
  if (glyphs['~']?.[1] && !glyphs['≈']) {          // ≈ is in no sheet: two of this hand's tildes
    const [adv, d] = glyphs['~'];
    const shift = (dy) => encode(decode(d).map((s) => s.map(([x, y]) => [x, y + dy])));
    glyphs['≈'] = [adv, `${shift(-90)};${shift(90)}`];
  }
  fonts.push([dir.name, glyphs]);
}
